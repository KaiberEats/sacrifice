...
> 前提：选做部分（hud 观测坍缩、pause、fourthwall）一律**视为要做**。
...
## 步骤四：全部 UI 牺牲（hud 观测坍缩 + pause + fourthwall 结局）（一次大会话，较重）
...
- **hud（观测坍缩）**：永久牺牲 hud 后状态 UI 解体消失；同时做一类"观测坍缩机关"——**HUD 可见时关闭（阻挡）、HUD 消失时开启（可通过）**。给它做成预制体，作为 B 之后能抄的样板。
- **fourthwall 结局**：最终祭坛触发后，界面元素逐个掉落/淡出、场景元素消解、一行收尾文字、黑屏。**纯视觉，不碰系统窗口。**

**建议提示词**：
> 分四块做 UI 牺牲。①把 HUD 改成图标版，每概念一图标三态，并让 HUD 监听 Sacrifice 信号。②实现暂停功能和 pause 牺牲：永久牺牲 pause 后暂停键失效。③实现 hud 牺牲：永久牺牲后状态 UI 解体消失；再做一个"观测坍缩机关"预制体，当 HUD 可见时它是实体阻挡、HUD 消失后变为可通过，做成可复用场景。④fourthwall 结局：最终祭坛触发后让 HUD 逐个掉落、场景元素淡出、显示一行结束文字然后黑屏，全程纯视觉不操作操作系统窗口。

**完成判定（玩一遍）**：
- [ ] HUD 图标随解锁/激活/槽位变化正确三态显示。
- [ ] 能暂停；牺牲 pause 后暂停键无效。
- [ ] 牺牲 hud 后状态 UI 消失；此前挡路的"观测坍缩机关"现在可以通过了。
- [ ] 触发最终祭坛：界面和场景依次解体到黑屏，且窗口本身没被乱动。

**回归重测**：牺牲 hud 后靠记忆玩一小段，确认没有 UI 也能操作（gravity/blue 仍能切）。

**解锁**：B 得到了"观测坍缩机关"这个复杂 gimmick 样板。
...
**给 B（gimmick）**：
- [ ] 反应式物体样板：`BlueObject.tscn`（简单）+ 观测坍缩机关（复杂）各一个，能直接复制改 `concept_id`。
